...
export default function HospitalDashboard() {
  ...
  const loadData = () => {
    ...
    if (savedDevices) setMedicalDevices(JSON.parse(savedDevices))
    else {
  ...
  const handleAddDevice = () => {
    ...
    const newDevice = { id: Date.now(), ...deviceForm, quantity: parseInt(deviceForm.quantity) || 1, price: parseFloat(deviceForm.price) || 0 }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div><h1 className="text-3xl font-bold gradient-text">لوحة التحكم الرئيسية</h1><p className="text-gray-400 mt-1">نظرة شاملة على أداء المستشفى</p></div>
        <div className="flex gap-2">
          <button onClick={refreshData} className="bg-green-500/20 hover:bg-green-500/30 text-green-400 px-4 py-2 rounded-xl flex items-center gap-2 border border-green-500/30"><RefreshCw size={18} /> تحديث البيانات</button>
          <button onClick={() => { setDeviceForm({ name: '', category: 'diagnostic', status: 'available', quantity: 1, purchaseDate: '', warrantyUntil: '', manufacturer: '', price: 0, notes: '' }); setEditingDevice(null); setShowDeviceModal(true); }} className="bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-400 px-4 py-2 rounded-xl flex items-center gap-2 border border-indigo-500/30"><Plus size={18} /> إضافة جهاز</button>
          <button onClick={() => window.print()} className="bg-blue-500/20 hover:bg-blue-500/30 text-blue-400 px-4 py-2 rounded-xl flex items-center gap-2 border border-blue-500/30"><Printer size={18} /> طباعة</button>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-gradient-to-br from-blue-500/20 to-blue-600/20 rounded-2xl p-5 border border-blue-500/30">
          <div className="flex items-center justify-between"><div><p className="text-gray-400 text-sm">إجمالي الإيرادات</p><p className="text-3xl font-bold text-white">${totalRevenue.toLocaleString()}</p></div><div className="p-3 bg-blue-500/20 rounded-xl"><DollarSign className="text-blue-400" size={28} /></div></div>
        ...
        <div className="bg-gradient-to-br from-purple-500/20 to-purple-600/20 rounded-2xl p-5 border border-purple-500/30">
          <div className="flex items-center justify-between"><div><p className="text-gray-400 text-sm">الأجهزة المتاحة</p><p className="text-3xl font-bold text-white">{availableDevices}</p></div><div className="p-3 bg-purple-500/20 rounded-xl"><Activity className="text-purple-400" size={28} /></div></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-gray-800/50 rounded-2xl overflow-hidden border border-gray-700/50 lg:col-span-2">
          <div className="px-6 py-4 border-b border-gray-700/50 flex justify-between"><h2 className="text-xl font-bold text-white">قائمة الأجهزة الطبية</h2></div>
