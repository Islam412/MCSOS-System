...
export default function UsersManager() {
  ...
  const loadLocalUsers = () => {
    const savedUsers = getLocalData(STORAGE_KEYS.USERS)
      ...
      if (isOnline) {
        try {
          ...
          const response = await executeWithOfflineSupport(
            ...
            'users',
            getLocalData(STORAGE_KEYS.USERS)
          )

          // التحقق من هيكل الاستجابة
          if (response && response.users) {
            setUsers(response.users)
            localStorage.setItem(STORAGE_KEYS.USERS, JSON.stringify(response.users))
          } else if (response && Array.isArray(response)) {
            setUsers(response)
            localStorage.setItem(STORAGE_KEYS.USERS, JSON.stringify(response))
          } else {
            loadLocalUsers()
          }
        } catch (apiError) {
    ...
    try {
      ...
      if (isOnline) {
        try {
          ...
          newUserData = response.user || response
  ...
  // ========== حظر/إلغاء حظر مستخدم (مع دعم API) ==========
    ...
    try {
      ...
      if (isOnline) {
        try {
          ...
          console.warn('API block/unblock failed, updating locally:', apiError)
        }
      }

      const updated = users.map(u =>
        u.id === userId ? { ...u, status: newStatus, _syncPending: !isOnline } : u
      )
      saveUsers(updated)
      toast.success(newStatus === 'active' ? 'تم إلغاء حظر الحساب' : 'تم حظر الحساب')
    } catch (error) {
      toast.error(error.message || 'حدث خطأ في تحديث حالة المستخدم')
    }
  }

  // ========== حذف مستخدم (مع دعم API) ==========
  ...
  // ========== إعادة تعيين كلمة المرور (مع دعم API) ==========
  ...
  const handleSaveNewPassword = async () => {
    ...
    try {
      if (isOnline) {
        try {
          await usersService.resetPassword(selectedUser.id, resetPasswordData.newPassword)
        } catch (apiError) {
          console.warn('API reset password failed, updating locally:', apiError)
        }
      }

      // تحديث محلياً
      const updated = users.map(u =>
        u.id === selectedUser.id ? { ...u, password: resetPasswordData.newPassword, _syncPending: !isOnline } : u
      )
      saveUsers(updated)
  ...
  const exportUsers = () => {
    const exportData = users.map(user => ({
      ...
      'نوع الحساب': user.roleAr || user.role,
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-400 mt-1">
            جميع حسابات المستخدمين المسجلة في النظام
            {!isOnline && (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
        ...
        <div className="bg-purple-500/10 rounded-xl p-3 text-center">
          <p className="text-2xl font-bold text-purple-400">{stats.admin}</p>
          <p className="text-xs text-gray-400">مديرين</p>
        </div>
        <div className="bg-blue-500/10 rounded-xl p-3 text-center">
          <p className="text-2xl font-bold text-blue-400">{stats.doctor}</p>
          <p className="text-xs text-gray-400">أطباء</p>
        </div>
        <div className="bg-green-500/10 rounded-xl p-3 text-center">
          <p className="text-2xl font-bold text-green-400">{stats.reception}</p>
          <p className="text-xs text-gray-400">استقبال</p>
        </div>
        <div className="bg-yellow-500/10 rounded-xl p-3 text-center">
          <p className="text-2xl font-bold text-yellow-400">{stats.finance}</p>
          <p className="text-xs text-gray-400">مالية</p>
        </div>
        <div className="bg-pink-500/10 rounded-xl p-3 text-center">
          <p className="text-2xl font-bold text-pink-400">{stats.patient}</p>
          <p className="text-xs text-gray-400">مرضى</p>
        </div>
        <div className="bg-gray-500/10 rounded-xl p-3 text-center">
          <p className="text-2xl font-bold text-gray-400">{stats.user}</p>
      ...
      <div className="bg-gray-800/50 rounded-2xl overflow-hidden border border-gray-700/50">
        <div className="px-6 py-4 border-b border-gray-700/50">
          <h2 className="text-xl font-bold text-white">قائمة المستخدمين ({filteredUsers.length})</h2>
        </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-800/80">
              <tr className={`${isRTL ? 'text-right' : 'text-left'}`}>
                ...
                <th className="px-4 py-3 text-sm text-gray-300">المستخدم</th>
                <th className="px-4 py-3 text-sm text-gray-300">البريد الإلكتروني</th>
                <th className="px-4 py-3 text-sm text-gray-300">نوع الحساب</th>
                <th className="px-4 py-3 text-sm text-gray-300">رقم الجوال</th>
                <th className="px-4 py-3 text-sm text-gray-300">تاريخ التسجيل</th>
                <th className="px-4 py-3 text-sm text-gray-300">آخر تسجيل</th>
            ...
            <tbody className="divide-y divide-gray-700/50">
              {filteredUsers.length === 0 ? (
                ...
                filteredUsers.map((user, idx) => {
                  const isPending = user._syncPending === true
                  return (
                    <tr key={user.id} className="hover:bg-gray-700/30">
                      ...
                      <td className="px-4 py-3">
                        ...
                        <div className="text-xs text-gray-500">{user.nameEn || ''}</div>
                      </td>
                      <td className="px-4 py-3 text-gray-300 dir-ltr text-sm">{user.email}</td>
                      <td className="px-4 py-3">{getRoleBadge(user.role)}</td>
                      <td className="px-4 py-3 text-gray-300 dir-ltr">{user.phone || '—'}</td>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-2xl max-w-md w-full p-6 border border-gray-700">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setShowAddUserModal(false)} className="p-1 hover:bg-gray-700 rounded">
            ...
            <div className="space-y-4">
              <input
                ...
                className="w-full p-2 bg-gray-700 rounded-lg text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"
                value={newUser.name}
      ...
      {showResetPasswordModal && selectedUser && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-2xl max-w-md w-full p-6 border border-gray-700">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-white">تغيير كلمة المرور - {selectedUser.name}</h2>
