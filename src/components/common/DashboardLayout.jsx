      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
        ...
        {isMobile && (
          <button
            ...
            className={`fixed top-4 z-50 p-3 rounded-xl bg-gray-800/80 backdrop-blur-md shadow-lg hover:bg-gray-700 transition-all duration-300 ${isRTL ? 'right-4' : 'left-4'}`}
          >
            {sidebarOpen ? <X size={24} className="text-blue-400" /> : <Menu size={24} className="text-blue-400" />}
          </button>
        )}

        {/* السايد بار - ثابت الحجم */}
        <div
          className={`
            fixed top-0 z-40 h-full bg-gray-800/95 backdrop-blur-md shadow-2xl flex flex-col transition-all duration-300
        ...
        {/* المحتوى الرئيسي - مع مسافة مناسبة للسايد بار */}
        <main 
          className={`
            min-h-screen transition-all duration-300
            ${!isMobile && !isRTL ? 'ml-80' : ''}
            ${!isMobile && isRTL ? 'mr-80' : ''}
          `}
        >
